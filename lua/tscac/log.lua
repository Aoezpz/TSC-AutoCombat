--[[
tscac/log.lua -- file-backed diagnostic logger for TSC AutoCombat
...
Three jobs:
  ...
  2. A print() hook that tees every chat line AutoCombat (and its plugins) prints
...
function M.filePath()
    return M.resolveDir() .. '/tscac_' .. identityTag() .. '.log'
end

function M.dumpPath()
    return M.resolveDir() .. '/tscac_dump_' .. identityTag() .. '_' .. os.date('%Y%m%d_%H%M%S') .. '.log'
end
...
local function writeHeader(f)
    local server, char = 'unknown', 'unknown'
    pcall(function() server, char = st.identity() end)
    f:write(string.format('==== TSC AutoCombat v%s log session %s | %s @ %s ====\n',
...
-- how AutoCombat marks its error lines, so classify those as warnings.
