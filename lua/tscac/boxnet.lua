...
-- ============================================================================
-- lua/tscac/boxnet.lua — AutoCombat Box Network Plugin (MacroQuest Actors)
-- ============================================================================
...
--   * Peer roster: every box running AutoCombat broadcasts a 1s heartbeat with
...
--     (the script's Lua state tears it down when AutoCombat stops).
-- ============================================================================
...
local MAILBOX            = 'tscac_boxnet'
local REGISTRY_KEY       = '__TSCAC_BOXNET_ACTOR'   -- _G slot holding the once-per-state dropbox
local INBOX_MAX          = 500
local LOG_MAX            = 60
...
local function registerActor()
    ...
    -- client (e.g. another AutoCombat instance in the same process).
...
-- runtime.enqueueBoxBuffRequest in tscac.lua). Nothing to do with Buffbot.
...
local function probeHint()
    ...
    elseif st == 'no answer' then
        ...
        return 'No reply from the launcher. Either this client is not attached to MacroQuest.exe (launcher window -> Actors panel should list this character) or the mailbox is a stale duplicate: run /lua run tscac again (a fresh Lua state), then /ac net probe.'
    end
    return nil
end
...
local function processMessage(message)
    ...
    if payload.v ~= PROTOCOL_VERSION then
        ...
        if not net.versionWarned[key] then
            ...
            logEvent(string.format('%s speaks protocol v%s (we are v%d) - update AutoCombat on both boxes', from, tostring(payload.v), PROTOCOL_VERSION), 'warn')
        end
        return
    end
...
local function drawStatusLine(GOOD, WARN, ERR, MUTED)
    ...
    elseif not warn and n == 0 and pst == 'ok' and (nowSec() - (net.startedAt or 0)) > LAUNCHER_HINT_SEC then
        warn = 'Launcher routing works but no other box has answered: make sure AutoCombat (with this plugin) is running on them.'
    end
...
local function drawWindow()
    ...
    local open, draw = ImGui.Begin('AutoCombat Box Network###TscacBoxNet', ctrl.show_boxnet, core.windowFlags and core.windowFlags('boxnet', windowFlags) or windowFlags)
    if not open then
